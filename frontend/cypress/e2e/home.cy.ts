...
describe('HomePage Tests', () => {
  ...
  it('handles date changes correctly', () => {
    ...
    cy.get("[data-test-id='startDate']", { timeout: 10000 }).should('be.visible').first().as('startDatePicker');

    cy.get("[data-test-id='endDate']").should('be.visible').last().as('endDatePicker');

    // Interact with the date pickers
    cy.get('@startDatePicker').click().clear().type(startDate + '{enter}');
    cy.get('@endDatePicker').click().clear().type(endDate + '{enter}');

    // Assert the values are updated
    cy.get('@startDatePicker').find('input').should('have.value', startDate);
