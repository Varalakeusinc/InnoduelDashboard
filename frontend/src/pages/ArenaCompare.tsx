...
const ArenaCompare = ({ onClose, isCloseDisabled }: { onClose: () => void; isCloseDisabled: boolean; }) => {
    ...
    const [notification, setNotification] = useState<{ id: string, notificationType: NotificationType; title: string; description: string; actionText: string; onActionClick: () => void; }[]>([]);
            ...
            <div className="flex flex-col space-y-4">
                <div className="flex justify-center">
                    ...
                    <div className="w-1/3 pl-2">
                        {!isCloseDisabled && (
                            <Button variant="outline" size="icon" onClick={() => onClose()} disabled={isCloseDisabled}>
                                <X className="h-4 w-4" />
                            </Button>
                        )}
                    </div>
                ...
                <div className="mt-2">
                    {comparisonData.length > 0 && selectedArena1 && selectedArena2 && <PlotGraph arena1={selectedArena1} arena2={selectedArena2} comparisonData={comparisonData} plotColor={plotColor} />}
                </div>
            </div>
