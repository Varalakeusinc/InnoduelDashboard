		...
		<div className="container mx-auto" style={{ padding: "0" }}>
			<div className="fixed top-40 right-4 z-50 flex space-x-2">
				...
				<Button
					variant="outline"
					size="icon"
					onClick={() => handleRemoveView(views[views.length - 1])}
			...
			<div className="mt-20 grid grid-flow-row md:grid-flow-col gap-4">
				{views.map((viewId, index) => (
					<div
						key={viewId}
						className={`inline-block ${index !== 0 ? "ml-4" : ""} ${
							index !== views.length - 1 ? "mr-4" : ""
						}`}
						style={{ width: "900px" }}
					>
						<div className="h-full">
							<ArenaCompare
								onClose={() => handleRemoveView(viewId)}
								isCloseDisabled={views.length === 1}
							/>
						</div>
					</div>
				))}
