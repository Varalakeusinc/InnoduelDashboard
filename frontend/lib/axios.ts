...
instance.interceptors.response.use(
    ...
    (error: AxiosError) => {
        ...
        if (axios.isAxiosError(error)) {
            ...
            console.error('\x1b[94m%s\x1b[0m', 'Axios error occurred:', axiosError.message);
            // Log the response data if available
            if (axiosError.response?.data) {
                console.error('\x1b[94m%s\x1b[0m', 'Request failed with data:', axiosError.response.data);
            }
