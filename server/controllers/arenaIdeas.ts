...
export const getIdeasOfArena = async (req: Request, res: Response) => {
  ...
  const { sort, order, min_votes } = req.query;

  const minVotesThreshold = min_votes ? parseInt(min_votes as string) : 0;

  try {
    const arena = await prisma.arena.findUnique({
      where: {
        id: parseInt(arena_id)
      },
      include: {
        idea: {
          select: {
      ...
      switch (sort) {
        case 'win_rate':
          ideas = ideas.sort((a, b) => {
            ...
            if (a.win_rate === null) return order === 'asc' ? 1 : -1;
            if (b.win_rate === null) return order === 'asc' ? -1 : 1;
            return order === 'asc' ? a.win_rate - b.win_rate : b.win_rate - a.win_rate;
          });
          break;
        case 'total_votes':
